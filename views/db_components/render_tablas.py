...
def get_tablas_ui(db_sel, es_admin, permisos, tiene_acceso, ir_a_nivel, get_style, cache, page, datos_filtrados=None):
    ...
    if es_admin:
        ...
        controls.append(ft.Container(content=ft.Column([
            ...
            ft.ElevatedButton("Guardar Tabla", bgcolor=ft.Colors.BLUE_700, color="white", 
                on_click=lambda e: [
                    crear_tabla(db_sel, input_t.value, [{"nombre": r.controls[0].value, "tipo": r.controls[1].value} for r in cols_ui.controls if r.controls[0].value]), 
    ...
    if es_admin or tiene_acceso("SELECT"):
        ...
        list_items.append(ft.Text("No tienes permisos de lectura en esta base de datos.", color="red", italic=True))
